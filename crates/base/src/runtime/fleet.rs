...
//!
//! `base` owns the tick cadence; `agent` (`Supervisor`) and
//! `workspace` (`Dispatcher`) plug in concrete hooks behind these
//! traits. That keeps the one-way chain `agent → workspace → base`:
...
/// Implemented by `agent::Supervisor`.
pub trait SupervisedTask: Send {
    ...
    /// `workspace`'s `DEFAULT_FAILURE_LIMIT`; `Supervisor` overrides
    /// it from its `Task`.
...
/// Produced by `workspace::Dispatcher` via an adapter.
