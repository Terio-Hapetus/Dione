...
fn retired_session_frames_are_dropped() {
    use base::state::event_session_id;
...
fn worktree_status_derives_from_session() {
    ...
    let mut s = Store::default();
    let mut r = base::worktree::WorktreeRecord::new(Path::new("/repo"), "feat-a").unwrap();
...
fn format_review_notes_renders_items() {
    use base::state::{DiffNote, format_review_notes, resolve_range};
...
fn append_reply_targets_note_by_value() {
    use base::state::{DiffNote, append_reply};
...
fn parse_patch_lines_multi_hunk() {
    use base::state::parse_patch_lines;
