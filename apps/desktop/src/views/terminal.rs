...
impl TermState {
    ...
    fn ensure_alive(&mut self) {
        if !self.guest && !self.shell.is_alive() {
            ...
            if let Ok(shell) = workspace::HostProvider::new().shell(&cwd) {
                self.shell = shell;
            }
        }
    }
...
impl DioneApp {
    ...
    fn guest_ready(&self, slug: &str) -> bool {
        self.vm_states
            .get(slug)
            .is_some_and(|s| matches!(s, workspace::ContainerState::Running))
    }
    ...
    pub(crate) fn toggle_terminal(&mut self) {
        ...
        match workspace::HostProvider::new().shell(&cwd) {
            Ok(shell) => {
...
mod tests {
    ...
    impl workspace::ShellChannel for FakeShell {
